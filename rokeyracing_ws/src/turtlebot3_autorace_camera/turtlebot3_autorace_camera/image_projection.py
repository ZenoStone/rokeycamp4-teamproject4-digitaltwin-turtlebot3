...
class ImageProjection(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(
            namespace='',
            parameters=[
                ('camera.extrinsic_camera_calibration.top_x', 140, parameter_descriptor_top),
                ('camera.extrinsic_camera_calibration.top_y', 50, parameter_descriptor_top),
                ('camera.extrinsic_camera_calibration.bottom_x', 180, parameter_descriptor_bottom),
                ('camera.extrinsic_camera_calibration.bottom_y', 115, parameter_descriptor_bottom),
    ...
    def cbImageProjection(self, msg_img):
        ...
        if self.is_calibration_mode:
            ...
            # (기존 elif self.resolution=='640x480': 블록 바로 다음에 추가)

            elif self.resolution=='1280x720':
            # 1280x720 해상도에 맞게 좌표를 수정하여 사다리꼴을 그립니다.
                cv_image_calib = cv2.line(
                    cv_image_calib,
                    (640 - top_x, 540 - top_y),
                    (640 + top_x, 540 - top_y),
        ...
        # (기존 pts_src, pts_dst 설정하는 elif self.resolution=='640x480': 블록 바로 다음에 추가)
        ...
        if self.resolution=='320x240':  
            ...
            pts_dst = np.array([[128, 0],     # 좌상
                                [512, 0],     # 우상
                                [512, 480],   # 우하
                                [128, 480]    # 좌하
                            ])
        elif self.resolution=='1280x720':
            # 결과 이미지의 원하는 꼭짓점 좌표 (예: 입력과 비슷한 비율)
            pts_dst = np.array([[0, 0], [1280, 0], [1080, 720], [200, 720]])
        ...
        if self.resolution=='320x240':  
            ...
            # Homography 적용 결과 이미지 크기를 640x480으로 설정
            cv_image_homography = cv2.warpPerspective(cv_image_original, h, (640, 480))

            # 삼각형 영역 정의 (왼쪽 아래와 오른쪽 아래 블랙 마스킹)
            triangle1 = np.array([[0, 479], [0, 300], [128, 479]], np.int32)       # 왼쪽 하단
            triangle2 = np.array([[639, 479], [639, 300], [512, 479]], np.int32)   # 오른쪽 하단

            # 채우기 (검은색)
            black = (0, 0, 0)
            cv_image_homography = cv2.fillPoly(cv_image_homography, [triangle1, triangle2], black)
        
        # (기존 cv2.warpPerspective elif self.resolution=='640x480': 블록 바로 다음에 추가)
        elif self.resolution=='1280x720':
            # Homography 적용 결과 이미지 크기를 1280x720으로 설정
            cv_image_homography = cv2.warpPerspective(cv_image_original, h, (1280, 720))

            # pts_dst에 맞춰 삼각형 영역 재정의
            triangle1 = np.array([[0, 719], [0, 500], [200, 719]], np.int32)
            triangle2 = np.array([[1279, 719], [1279, 500], [1080, 719]], np.int32)
